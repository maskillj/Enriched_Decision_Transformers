if __name__ == '__main__': 
    ...
    """
    The training regime for the Decision Transformers deviates from the original presentaiton of the model because we
    are indifferent between different reward:[drawdown/episode length] pairs and concerned only with the model's ability 
    to achieve the target reward-drawdown pair reliably. We therefore select both values randomly from the interval of possible 
    values represented in the training data. Where multiple training datasets exists, these intervals are chosen as the 
    maximum and minimum from the union of dataset trajectories. It is expected that loss curves from these targets will better
    reflect the generalized control capability of the deicison transformer.
    (The only exception to this methodology is the ignorance of the 'Expert' dataset (where available) in reflection of the 
    fact that the maximum rewards and timesteps represented in their datasets are orders of magnitude greater than all the other
    trajectories comprising an environment's datasets.)
    """

    def train_drawdown(env):
        args = parse_args("config/walker/default_parameters.py")
        ...
        for i_epoch in range(1, args.max_epoch+1):
            ...
            if i_epoch % 2 == 0:
                eval_metrics = eval_decision_transformer(env, policy, rand(), args.return_scale, args.eval_episode, seed=args.seed, target_max_drawdown=rand())
